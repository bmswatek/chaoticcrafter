...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const { specialAbilities, actions } =
        formatMonsterDetails(monsterDetails);
      ...
      let detailsEmbed = buildDetailsEmbed(
        actionsPages[currentPage],
        "Actions",
        monsterDetails
      );
      ...
      collector.on("collect", async (i) => {
        if (i.customId === "next") {
          ...
          currentSection =
            currentSection === "actions" ? "special_abilities" : "actions";
          totalPages =
            currentSection === "actions"
              ? actionsPages.length
              : specialAbilitiesPages.length;
          currentPage = 0;
          row.components[2].setLabel(
            currentSection === "actions"
              ? "Toggle to Special Abilities"
              : "Toggle to Actions"
          );
        }
        ...
        detailsEmbed = buildDetailsEmbed(
          currentSection === "actions"
            ? actionsPages[currentPage]
            : specialAbilitiesPages[currentPage],
      ...
      await interaction.followUp({
        content:
          "Error executing random encounter command. Please try again later.",
        ephemeral: true,
      });
    }
  },
...
function formatMonsterDetails(monsterDetails) {
  const conditionImmunities =
    monsterDetails.condition_immunities?.map((obj) => obj.name).join(", ") ||
    "None";
  ...
  const skills =
    monsterDetails.proficiencies
      ...
      .map(
        (skill) => `${skill.proficiency.name.split(": ")[1]}: +${skill.value}`
      )
      .join(", ") || "None";
...
function buildMainEmbed(monsterDetails) {
  return new EmbedBuilder()
    ...
    .setColor(0xce3e43)
    .setDescription(
      `*${monsterDetails.size || "Unknown"} ${
        monsterDetails.type || "Unknown"
      }, ${monsterDetails.alignment || "Unknown"}*\n\n` +
        `**Armor Class**: ${
        ...
        `**STR**: ${monsterDetails.strength || "Unknown"}    **DEX**: ${
          monsterDetails.dexterity || "Unknown"
        }    **CON**: ${monsterDetails.constitution || "Unknown"}\n` +
        `**INT**: ${monsterDetails.intelligence || "Unknown"}    **WIS**: ${
          monsterDetails.wisdom || "Unknown"
        }    **CHA**: ${monsterDetails.charisma || "Unknown"}\n\n` +
        `**Saving Throws**: ${
          monsterDetails.proficiencies
            ?.filter((proficiency) =>
              proficiency.proficiency.index.startsWith("saving-throw")
            )
            .map(
              (proficiency) =>
                `${proficiency.proficiency.name.split(": ")[1]}: +${
                  proficiency.value
                }`
            )
            .join(", ") || "None"
        ...
        `**Senses**: Passive Perception ${
          monsterDetails.senses?.passive_perception || "Unknown"
        }\n` +
        `**Challenge Rating**: ${
          monsterDetails.challenge_rating || "Unknown"
        } (${monsterDetails.xp || "Unknown"} XP)\n\n` +
        `**Damage Vulnerabilities**: ${
          monsterDetails.damage_vulnerabilities?.join(", ") || "None"
        }\n` +
        `**Damage Resistances**: ${
          monsterDetails.damage_resistances?.join(", ") || "None"
        }\n` +
        `**Damage Immunities**: ${
          monsterDetails.damage_immunities?.join(", ") || "None"
        }\n` +
    ...
    .setImage(
      monsterDetails.image
        ? `https://www.dnd5eapi.co${monsterDetails.image}`
        : null
    );
}
...
function buildDetailsEmbed(page, title, monsterDetails) {
  return new EmbedBuilder()
    ...
    .setColor(0xce3e43)
    .setDescription(
      `${page.join("\n")}\n[API Entry](https://www.dnd5eapi.co${
        monsterDetails.url
      })`
    );
}
